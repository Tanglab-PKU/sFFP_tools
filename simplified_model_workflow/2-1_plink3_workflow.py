...
input_file = current_dir / 'plink3_BSA-SDA.csv'
input_df = pd.read_csv(input_file)
output_xlsx = current_dir / "plink3_output.xlsx"
MGF_CSV_PATH = current_dir / "test_data" / "mgf_to_csv"
# Create output directory for spectra
SPECTRUM_OUTPUT = current_dir / "plink3_Jsearch_spectrums"
SPECTRUM_OUTPUT.mkdir(exist_ok=True)


# Step 1: Filter peptides
filter_condition = (input_df['Peptide_Type'] == 3) & (input_df['Score'] > 0.3) & (~input_df['Title'].str.contains("_H_"))
processed_df = input_df.loc[filter_condition].copy()

# Step 2: Calculate peptide PSMs
processed_df['Peptide_PSMs'] = processed_df.groupby('Peptide')['Peptide'].transform('size')
